...
To initialize a project, please specify path for working directory on the snakefile_redPipe. You also need to create config.yaml which specify path for all sequencing read files for each samples as well as their corresponding population (see example/config.yaml). For testing, you can use test data (example/data/<three files with fq-suffix>).
